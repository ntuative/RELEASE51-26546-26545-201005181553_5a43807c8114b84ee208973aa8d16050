package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
      ...
      public function method_13() : uint
      {
      ...
      private function parseSignature(param1:IDataInput) : Boolean
      {
         ...
         if(param1.bytesAvailable >= 4)
         {
            ...
            {
               case 67324752:
                  var_95 = parseLocalfile;
                  var_517 = new FZipFile(this,charEncoding);
                  break;
               case 33639248:
               case 101010256:
                  var_95 = parseIdle;
                  break;
               default:
                  dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_133,"Unknown record signature."));
            }
            return true;
         }
         return false;
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_133))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_133,e.message));
         }
      }
      ...
      {
         if(!var_94 && var_95 == parseIdle)
         {
            var_48 = [];
            var_134 = new Dictionary();
            param1.position = 0;
            param1.endian = Endian.LITTLE_ENDIAN;
            var_95 = parseSignature;
            if(parse(param1))
            {
               var_95 = parseIdle;
               dispatchEvent(new Event(Event.COMPLETE));
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_133,"EOF"));
            }
         }
      }
      ...
      {
         if(var_517.parse(param1))
         {
            var_48.push(var_517);
            if(false)
            {
               var_134["null"] = var_517;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1303,var_517));
            var_517 = null;
            if(var_95 != parseIdle)
            {
               var_95 = parseSignature;
      ...
      {
         var _loc2_:* = null;
         if(var_48 != null && var_134 != null && param1 < var_48.length)
         {
