   ...
   public class DimmerViewColorGrid
   {
      ...
      private function populateColourGrid(param1:IHabboWindowManager) : void
      {
         ...
         for each(_loc2_ in colors)
         {
            ...
            _loc3_.width = var_547.width;
            _loc3_.height = var_547.height;
            var_453.addGridItem(_loc3_);
            _loc4_ = _loc3_.findChildByTag("BG_BORDER") as IBitmapWrapperWindow;
            if(_loc4_ != null)
            {
               _loc4_.bitmap = new BitmapData(var_547.width,var_547.height,true,0);
               _loc4_.bitmap.copyPixels(var_547,var_547.rect,new Point(0,0));
            }
            _loc5_ = _loc3_.findChildByTag("COLOR_IMAGE") as IBitmapWrapperWindow;
            if(_loc5_ != null)
            {
               _loc5_.bitmap = new BitmapData(var_932.width,var_932.height,true,0);
            ...
            if(_loc6_ != null)
            {
               _loc6_.bitmap = new BitmapData(var_767.width,var_767.height,true,16777215);
               _loc6_.bitmap.copyPixels(var_767,var_767.rect,new Point(0,0),null,null,true);
      ...
      {
         var _loc2_:int = var_453.getGridItemIndex(param1.target as IWindow);
      ...
      public function setSelectedColorIndex(param1:int) : void
      {
         ...
         if(param1 < 0 || param1 >= var_453.numGridItems)
         {
            return;
         }
         select(var_453.getGridItemAt(param1) as IWindowContainer);
      }
