package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(var_1549 == RoomWidgetRoomQueueUpdateEvent.const_337)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_565);
         }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            var_1549 = param1.type;
            var_1074 = false;
            var_1073 = param1.position;
         }
         else
         {
            var_1074 = true;
         }
         var_2082 = param1.hasHabboClub;
         var_1548 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",var_1073.toString());
         localizations.registerParameter("room.queue.position.hc","position",var_1073.toString());
         localizations.registerParameter("room.queue.spectator.position","position",var_1073.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",var_1073.toString());
         showInterface();
      }
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_337,onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_337,onQueueStatus);
      ...
      {
         messageListener.processWidgetMessage(new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_758));
      }
         ...
         if(_loc1_ != null && _loc2_ != null && _loc3_ != null)
         {
            switch(var_1549)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_337:
                  _loc1_.caption = !!var_1548 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = var_1074;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_439:
                  _loc1_.caption = !!var_1548 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
