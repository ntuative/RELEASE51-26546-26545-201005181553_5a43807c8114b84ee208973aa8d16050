package com.sulake.core.window.graphics
{
   ...
   public class WindowRenderer implements IWindowRenderer
   {
      ...
      private var var_1625:Rectangle;
      ...
      public function WindowRenderer(param1:ISkinContainer)
      {
         ...
         var_652 = new Array();
         var_1626 = new Point();
         var_1625 = new Rectangle();
         var_329 = new Rectangle();
      ...
      public function render() : void
      {
         ...
         while(_loc4_ < _loc2_)
         {
            _loc1_ = var_652.pop() as WindowController;
            if(!_loc1_.disposed)
            {
               _loc3_ = getWindowRendererItem(_loc1_);
               var_329.x = _loc3_.dirty.x;
               var_329.y = _loc3_.dirty.y;
               var_329.width = _loc3_.dirty.width;
               var_329.height = _loc3_.dirty.height;
               renderWindowBranch(_loc1_,_loc3_,var_329);
            }
            _loc4_++;
         }
      }
      ...
      {
         param1.removeEventListener(WindowDisposeEvent.const_669,windowDisposedCallback);
         var _loc2_:WindowRendererItem = var_328[param1] as WindowRendererItem;
         if(_loc2_ != null)
         {
            _loc2_.dispose();
      ...
      private function renderWindowBranch(param1:WindowController, param2:WindowRendererItem, param3:Rectangle) : void
      {
         ...
         if(param1.visible)
         {
            if(getDrawLocationAndClipRegion(param1,param3,var_1626,var_1625))
            {
               if(param2.render(param1,var_1626,var_1625))
               {
      ...
      public function registerRenderable(param1:IWindow) : void
      {
         ...
         if(_loc2_ == null)
         {
            _loc2_ = new WindowRendererItem(this,_drawBufferAllocator,var_692);
            var_328[param1] = _loc2_;
            param1.addEventListener(WindowDisposeEvent.const_669,windowDisposedCallback);
         }
      }
      ...
      public function addToRenderQueue(param1:IWindow, param2:Rectangle, param3:uint) : void
      {
         ...
         if(getWindowRendererItem(param1).invalidate(param1,param2,param3))
         {
            ...
            var_329.x = param2.x - param1.x;
            var_329.y = param2.y - param1.y;
            var_329.width = param2.width;
            var_329.height = param2.height;
            if(param1.testParamFlag(WindowParam.const_35))
            {
               while(param1.testParamFlag(WindowParam.const_35))
               {
                  ...
                  var_329.offset(param1.x,param1.y);
                  param1 = _loc4_;
               }
            }
            getWindowRendererItem(param1).invalidate(param1,var_329,WindowRedrawFlag.const_278);
            if(var_652.indexOf(param1) == -1)
            {
               var_652.push(param1);
            }
         }
